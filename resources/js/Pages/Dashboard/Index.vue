...
<template>
    ...
    <AuthenticatedLayout title="Dashboard">
        ...
        <section class="mt-6 grid gap-6 xl:grid-cols-5">
            <div class="panel xl:col-span-3">
                <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                    <div>
                        <h3 class="font-bold text-slate-950">Últimos tickets</h3>
                        <p class="text-xs text-slate-500">Actividad reciente del soporte</p>
                    </div>
                    <Link
                ...
                <div v-if="latestTickets.length" class="divide-y divide-slate-100">
                    <Link
                        v-for="ticket in latestTickets"
                        :key="ticket.id"
                        :href="route('tickets.show', ticket.id)"
                        class="flex flex-col gap-3 px-5 py-4 transition hover:bg-slate-50 sm:flex-row sm:items-center"
                    >
                ...
                <div class="space-y-4">
                    <div v-for="(label, key) in statusLabels" :key="key">
                        <div class="mb-1.5 flex justify-between text-xs font-semibold">
                            <span>{{ label }}</span>
                            <span>{{ ticketsByStatus[key] || 0 }}</span>
                        </div>
                        <div class="h-2.5 overflow-hidden rounded-full bg-slate-100">
                            <div
                                :class="['h-full rounded-full', statusColors[key]]"
                                :style="{
                                    width: `${Math.min(
                                        100,
        ...
        <section class="mt-6 grid gap-6 xl:grid-cols-2">
            <div class="panel">
                <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                    <h3 class="font-bold text-slate-950">Próximo mantenimiento</h3>
                    <Link
                        :href="route('maintenance.index', { schedule: 'upcoming' })"
                        class="text-sm font-semibold text-indigo-600"
                    >
                        Agenda →
                    </Link>
                </div>
                <div v-if="upcomingTasks.length" class="divide-y divide-slate-100">
                    <Link
                    ...
                    >
                        ...
                        <div class="h-3 overflow-hidden rounded-full bg-slate-100">
                            <div
                                ...
                                :style="{ width: `${(item.total / maxTech) * 100}%` }"
                            />
                        </div>
                        <span class="text-right text-xs font-bold">{{ item.total }}</span>
                    </div>
                </div>
                <EmptyState v-else />
            </div>
